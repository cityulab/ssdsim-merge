...
int  main(int argc, char* argv[])
{
    ...
    printf("channel No.|Clip No.|die No.|plane No.|free page No.|\n");
    for (i=0;i<ssd->parameter->channel_number;i++)
    {
        for(l=0;l<ssd->parameter->chip_channel[i];l++)
        {
...
int get_requests(struct ssd_info *ssd)
{
    ...
    sscanf(buffer,"%lld %d %d %d %d",&time_t,&device,&lsn,&size,&ope); // 时间，设备号，逻辑扇区号，尺寸，读(1)/写(0)
    printf("[trace: %lld %d %d %d %d]\n",time_t,device,lsn,size,ope);
    if ((device<0)&&(lsn<0)&&(size<0)&&(ope<0))
    {
        return 100; // 100 是一个错误魔术数，会结束trace的循环
    }
    // 限制最小的扇区号
    if (lsn<ssd->min_lsn)
        ssd->min_lsn=lsn;
    ...
    ***********************************************************************************************************/
    ...
    {
        printf("[hubery-get_requests] -----------> does not equal MAX_INT64 update current time\n");
    ...
    if(ssd->request_queue == NULL)          //The queue is empty 添加到链表当中
    {
        printf("[hubery-get_requests] -----------> create and insert into a new queue\n");
    ...
    {
        printf("[hubery-get_requests] -----------> insert into the request queue\n");
        (ssd->request_tail)->next_node = request1; // 添加到链表当中
        ssd->request_tail = request1;
        ssd->request_queue_length++;
    }
    ...
    {
        // 平均读尺寸 = (累计平均读尺寸 * 累计请求数目 + 这次请求尺寸) / 累计请求数目 + 1
    ...
    ssd->next_request_time = time_t; // 读取第二行trace的的请求时间
    fseek(ssd->tracefile,filepoint,0); //文件指针返回到filepoint的位置
    ...
    if(new_request->operation == READ)
    {
        while(lpn<=last_lpn)
        {
            ...
            *************************************************************************************************/
            ...
            index=(lpn-first_lpn)/(32/ssd->parameter->subpage_page); // 计算属于哪个标志位
 ...
 * 读数据准备好了
 *
 * 每个channel可以称为一个独立单元，每一个channel可以进行并发的访问，同时也可以拥有自己的队列，
 * 每个独立单元有四个状态：当前状态，当前状态时间，下一状态，下一状态时间
 *
 * channel状态标志：含义：停留时间
 * CHANNEL_IDLE : 空闲 : 0~endless
 * CHANNEL_C_A_TRANSFER : 命令地址传输 : 7 * tWC
 * CHANNEL_DATA_TRASFER : 数据传输 : 数据量 * tRC
 *
 * clip状态标志：含义：停留时间
 * CHIP_IDLE : 空闲 : 0~endless
 * CHIP_WRITE_BUSY : 写操作 : tPROC
 * CHIP_READ_BUSY : 读操作 : tR
 * CHIP_C_A_TRANSFER : 命令地址传输 : 7 * tWC
 * CHIP_DATA_TRANSFER : 数据传输 : 数据量 * tRC
 *
 * 已知每个独立单元的当前状态，和当前状态时间，
 * 同时下一状态和下一状态的预计时间可以通过状态转换图和状态参数表获得，
 * 在所有独立单元中寻找最先到达的下一个状态的时间，作为SSDsim的新的系统时间，
 * 周而复始地使模拟不断向前推进。通过对所模拟的固态盘内的每个状态进行观测，
 * 可以准确地得到对每条外部请求的响应时间，从而完成SSDsim的时间模拟
 *
***********************************************************************************************************/
int64_t find_nearest_event(struct ssd_info *ssd)
{
    unsigned int i,j;
    int64_t time=MAX_INT64; // 64位max值，没有的意义的值
    int64_t time1=MAX_INT64;
    int64_t time2=MAX_INT64;

    for (i=0;i<ssd->parameter->channel_number;i++) // 循环所有channel，看看是否有最新时间发生(>current time)
    {
        if (ssd->channel_head[i].next_state == CHANNEL_IDLE) // 初始化的时候是idle
            if(time1 > ssd->channel_head[i].next_state_predict_time)
                if (ssd->channel_head[i].next_state_predict_time > ssd->current_time) // next_state_predict_time 通过memset 都设置为0
                    time1 = ssd->channel_head[i].next_state_predict_time; // 通过循环找到最接近current time的ssd->channel
        for (j=0;j<ssd->parameter->chip_channel[i];j++)
        {
            if ((ssd->channel_head[i].chip_head[j].next_state == CHIP_IDLE)||(ssd->channel_head[i].chip_head[j].next_state == CHIP_DATA_TRANSFER)) // 初始化时idle
                if(time2 > ssd->channel_head[i].chip_head[j].next_state_predict_time)
                    if (ssd->channel_head[i].chip_head[j].next_state_predict_time > ssd->current_time)
                        time2 = ssd->channel_head[i].chip_head[j].next_state_predict_time; // 找到最接近current time的clip
        }
    }
