...
*******************************************/
struct ssd_info *initiation(struct ssd_info *ssd, const char * tracefile, const char * paramfile, const char * outfile)
{
	...
	strncpy(ssd->parameterfilename,paramfile,16); //paramfile page.parameters
	//strncpy(ssd->tracefilename,"example.ascii",25);
	// printf("\ninput trace file name:");
    strcpy(ssd->tracefilename, tracefile);
	//scanf("%s",ssd->tracefilename);
	strncpy(ssd->outputfilename,outfile,7);//outfile ex.out
...
struct parameter_value *load_parameters(char parameter_file[30])
{
	...
	while(fgets(buf,200,fp)){
		...
		if((res_eql=strcmp(buf,"chip number")) ==0){
			...
			sscanf(buf + next_eql,"%d",&p->subpage_page); // defulat = 4
